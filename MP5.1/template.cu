...
__global__ void total(float *input, float *output, int len) {
    ...
    if (t < len) partialSum[t] = input[start + t];
    else partialSum[t] = 0;

    if (blockDim.x + t < len) partialSum[blockDim.x + t] = input[start + blockDim.x + t];
...
int main(int argc, char **argv) {
    ...
    hostOutput = (float *)malloc(outputSize);
    ...
    dim3 dimGrid(ceil(numInputElements / (BLOCK_SIZE * 2.0)));
